...
#m : total number of stops  (positive integer)

#dist.go:     distribution of go trials    (ExG or SW)
#dist.stop:   distribution of stop.trials  (ExG or SW)

#theta.go=c(mu.go,sigma.go,tau.go)
#theta.stop=c(mu.stop,sigma.stop,tau.stop)

#SSD.b:  one block starting stop signal delay

SstSimulatedTrackingMethodBlock=function(pid,n,m,SSD.b,dist.go,theta.go,dist.stop,theta.stop){
  ...
  if (GORT[1] > SSRT[1]+SSD[1]) { TrialType[1]<- 'Stop(Successful)' } else  { TrialType[1]<- 'Stop(Failed)' }
  if (GORT[1] > SSRT[1]+SSD[1]) { Inhibition[1]<- '1' } else  { Inhibition[1]<- '0' }
  if (GORT[1] > SSRT[1]+SSD[1]) {  index[1]<-  +50 } else  { index[1]<- -50}
  if (GORT[1] > SSRT[1]+SSD[1]) {  SSD[2]<-  SSD[1]+50 } else  { SSD[2]<- SSD[1]-50}
  ...
  for (i in 1:(m-1))
  {
    ...
    if (index[i]== 50)  {if (GORT[i+1] > SSRT[i+1]+SSD[i+1]) { TrialType[i+1]<- 'Stop(Successful)' }
      else  {TrialType[i+1]<- 'Stop(Failed)'}  }
    if (index[i]== 50)  {if (GORT[i+1] > SSRT[i+1]+SSD[i+1]) {  index[i+1]<- +50        }
      else  { index[i+1]<- -50       }  }
    ...
    if (index[i]== 50)  {if (GORT[i+1] > SSRT[i+1]+SSD[i+1]) { Inhibition[i+1]<- '1'} 
  ...
  for(i in 1:m){ MAT22[2*i-1,]<-MAT2[i,]; MAT22[2*i,]<-MAT2[m+i,]}
...
SSD.b=200
dist.go="ExG"
theta.go=c(400,60,30)
dist.stop="ExG"
theta.stop=c(100,70,60)
myblockSSTdata1<-SstSimulatedTrackingMethodBlock(pid,n,m,SSD.b,dist.go,theta.go,dist.stop,theta.stop)
...
SSD.b=200
dist.go="SW"
theta.go=c(100,0.01,50)
dist.stop="SW"
theta.stop=c(75,0.01,100)
myblockSSTdata2<-SstSimulatedTrackingMethodBlock(pid,n,m,SSD.b,dist.go,theta.go,dist.stop,theta.stop)
...
dist.stop="SW"
theta.stop=c(75,0.01,100)
myblockSSTdata3<-SstSimulatedTrackingMethodBlock(pid,n,m,SSD.b,dist.go,theta.go,dist.stop,theta.stop)
...
#SSD.b:  a vector of size b of starting stop signal delay

#Output: a giant matrix with "sum(n)" rows and (7+1) columns

SstSimulatedTrackingMethod=function(pid,n,m,SSD.b,dist.go,theta.go,dist.stop,theta.stop) { 
  b<-length(block)
  csn<-c(0,cumsum(n))
